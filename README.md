...
This project estimates decadal shoreline change and associated uncertainty around Aotearoa New Zealand by combining: 

- satellite-derived shoreline positions and beach slopes,
- regional sea-level rise scenarios from NZ SeaRise,
- historical shoreline trend information from the observed record, and
- a Monte Carlo propagation framework to turn these components into probabilistic shoreline projections.

The workflow is designed to produce site-specific projections for future shoreline position under different sea-level rise scenarios, while explicitly retaining the uncertainty associated with both the historical shoreline trend and the future sea-level forcing.
...
The current workflow follows a combined, site-based projection structure:

1. Input data are assembled for each site: shoreline positions, beach slope, and local SLR scenario values.
2. Historical shoreline evolution is reconstructed from the observed record, with smoothing and resampling used to retain the underlying trend while controlling temporal noise.
3. A local sea-level rise response is computed using the beach profile geometry and the relevant SLR forcing.
4. The historical trend component and the sea-level-rise-driven component are combined within a Monte Carlo framework.
...
Uncertainty is propagated through the model using repeated stochastic draws from the relevant forcing and trend distributions. The final product retains a range of possible outcomes around the median projection, with lower and upper bounds indicating the expected spread in future shoreline response.

The method is therefore designed to provide an estimate of the likely envelope of shoreline change, rather than a single deterministic future position.

## 3.2 Output products

The model generates projected shoreline change for each site and future period, together with uncertainty bounds. At a high level, the outputs provide at each site:

- a median shoreline-change projection,
- the range of plausible outcomes under the scenario ensemble,
- the slr-only component of shoreline change
- the continuation of the shoreline's local behaviour

This overview is intentionally high level; the detailed implementation and methodology can be seen in this pdf.
