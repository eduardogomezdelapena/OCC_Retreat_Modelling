...
RUN_CONFIG = {
    "site_ids_override": ["nzd0338-0000"], # "nzd0325" # Non-empty list overrides region-based site selection.
...
def filter_to_longest_consecutive_year_run(
    ...
    for start_idx, end_idx in zip(segment_starts[::-1], segment_ends[::-1]):
        ...
        found_segment_with_must_bepresent_year = True
    ...
    if selected_segment is None:
        if found_segment_with_must_bepresent_year:
            meta["status"] = f"insufficient_recent_span_with_{must_bepresent_year}"
        else:
            meta["status"] = f"no_segment_with_{must_bepresent_year}"
...
#%
        ...
        # Match the Bruun-rule convention used elsewhere in the repository:
        # the active-profile slope is scaled by 0.5 before use.
